...
from custom_data_transform import Sentinel1Transform
...
def save_dataset(raw_ds, keys, image_size, filename):
    ...
    image_dims = (41, 2, image_size, image_size,)
    chunk_size = 1000
...
def main(args):
    ...
    if args.region == 'south-africa':
        tmp = os.path.join(args.raw_data_dir, 'ref_fusion_competition_south_africa_train_source_sentinel_1')
    ...
    # Get the transformer
    # TODO provide the Sentinel1Transform
    transform = Sentinel1Transform(spatial_encoder=args.t_spatial_encoder,
    ...
    for source, label, target, comment, is_train in zip(list_source, list_labels, list_targets, list_comment, list_is_train):
        ...
        reader = CS1Reader.S1Reader(input_dir=source,
                                    label_dir=label,
                                    output_dir=target,
                                    label_ids=label_ids,
...
if __name__=='__main__':
    ...
    parser.add_argument('--target-data-dir', type=str,
                        default='/work/ka1176/shared_data/2021-ai4food/dev_data/sentinel-1/default/')
